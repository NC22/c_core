...
uses Windows,SysUtils,idHTTP,IdBaseComponent , IdComponent,
  IdTCPConnection, IdTCPClient, IdAntiFreeze,IdMultipartFormData,Classes,AbUtils,AbArcTyp,AbUnZper,ShlObj, MD5 , c_crypt;
...
type
  ...
  TCore = Class
    private 
	 ...
	 procedure HTTPmechWork(Sender: TObject; AWorkMode: TWorkMode; const AWorkCount: Integer);
	 procedure HTTPmechWorkBegin(Sender: TObject; AWorkMode: TWorkMode; const AWorkCountMax: Integer);
	 procedure HTTPmechWorkEnd(Sender: TObject; AWorkMode: TWorkMode);
         procedure UnzipItemWork(Sender: TObject; Item : TAbArchiveItem; Progress : Byte; var Abort : Boolean);
         procedure UnzipItemFail(Sender: TObject; Item : TAbArchiveItem; ProcessType : TAbProcessType; ErrorClass : TAbErrorClass; ErrorCode : Integer);
    ...
    protected
	 ...
	 procedure ExecEventUnarchItem(Item : TAbArchiveItem; Progress : Byte; var Abort : Boolean); dynamic;
         procedure ExecEventFailItem(Item : TAbArchiveItem; ProcessType : TAbProcessType; ErrorClass : TAbErrorClass; ErrorCode : Integer); dynamic;
    public
         property OnDownloadProcess : TDwnEvent read FOnDwnProcess write FOnDwnProcess;
         ...
         property OnUnzipItemFail : TAbArchiveItemFailureEvent read FOnFailItem write FOnFailItem;
         property OnUnarchItem : TAbArchiveItemProgressEvent read FOnUnarchItem write FOnUnarchItem;
	 ...
	 function Login : boolean;
         ...
         Constructor Create(PROG : string = 'TinyLauncher'; GameDirName : string = 'TWEBMCR');
...
Constructor TCore.Create(PROG : string = 'TinyLauncher'; GameDirName : string = 'TWEBMCR');
var
 tmp: string;
begin
   Inherited Create;
   ...
   if (Length(GameDirName) = 0) then begin
   ...
   if (Length(Options.gOptions.Game) = 0) then begin

        if not DirectoryExists(tmp+'\.'+ GameDirName +'\') then CreateDirectory(PAnsiChar(tmp+'\.'+ GameDirName +'\'), nil);
...
begin
 ...
 Options.webOptions.Distr_list := 'minecraft.jar|jinput.jar|lwjgl.jar|lwjgl_util.jar|windows_natives.jar';
...
begin
   ...
   for i:=6 to 9 do
        if (FileExists(tmp+'\Java\jre'+ IntToStr(i) +'\bin\javaw.exe')) then begin
           Options.gOptions.Java := tmp+'\Java\jre'+ IntToStr(i) +'\bin\javaw.exe';
           result := true;
           SaveOptions;
           exit
        end;

   if messageBox(0,'Программа не смогла найти библиотеки Java, открыть http://www.java.com/ru/download/ и скачать их прямо сейчас ? ','Внимание',MB_OKCANCEL or mb_iconquestion) = 1 then
   WinExec(PChar('rundll32 url.dll,FileProtocolHandler http://www.java.com/ru/download/'), SW_SHOWNORMAL);
end;
...
var
  Params,Launch,GameFiles : string;
...
begin
  ...
  if (Length(AuthInfo.RealLogin) = 0) or (Length(AuthInfo.SessionId) = 0) then PlayOnline := false;

  if (PlayOnline) and (TmpOptions.gOptions.MineHash <> 'none') and (NeedToFileUpdate(TmpOptions.gOptions.Game + 'bin\minecraft.jar', TmpOptions.gOptions.MineHash)) then
  exit;

  if Length(TmpOptions.pOptions.Login) = 0 then TmpOptions.pOptions.Login := 'Default';
  ...
  Launch := ' -Xms' + TmpOptions.gOptions.MinMem +
            ' -Xmx' + TmpOptions.gOptions.MaxMem +
            ' -Djava.library.path="'+TmpOptions.gOptions.Game+'bin\natives"' +
            ' -cp "'+ GameFiles +'"' +
            ' net.minecraft.client.Minecraft '+Params;

  if Length(TmpOptions.gOptions.Java) = 0 then begin
     if FindJava then TmpOptions := PrivateGetOptions
     else exit;
  end; 

  WinExec(PChar(TmpOptions.gOptions.Java+Launch),SW_SHOW);
...
begin
  if Assigned(FBadLogin) then FBadLogin();
end;

procedure TCore.ExecEventUnknown(Response : string);
begin
  if Assigned(FUnknownErr) then FUnknownErr(Response);
end;

procedure TCore.ExecEventConnectErr();
begin
  if Assigned(FConnErr) then FConnErr();
end;

procedure TCore.ExecEventOldVer();
begin
  if Assigned(FOldVer) then FOldVer();
end;

procedure TCore.ExecEventOldLauncher();
begin
  if Assigned(FOldEvent) then FOldEvent();
end;

procedure TCore.ExecEventFailItem(Item : TAbArchiveItem; ProcessType : TAbProcessType; ErrorClass : TAbErrorClass; ErrorCode : Integer);
begin
  if Assigned(FOnFailItem) then FOnFailItem(Self, Item, ProcessType, ErrorClass, ErrorCode);
end;

procedure TCore.ExecEventDwnProcess(DwnFile: downloadInfo);
begin
  if Assigned(FOnDwnProcess) then FOnDwnProcess(DwnFile,Self);
end;

procedure TCore.ExecEventDwnEnd(DwnFile: downloadInfo);
begin
  if Assigned(FOnDwnEnd) then FOnDwnEnd(DwnFile,Self);
end;

procedure TCore.ExecEventUnarchItem(Item : TAbArchiveItem; Progress : Byte; var Abort : Boolean);
begin
  if Assigned(FOnUnarchItem) then FOnUnarchItem(Self,Item,Progress,Abort);
end;
...
begin
  ...
  if (Length(TmpOptions.pOptions.Login) = 0) and (Length(TmpOptions.pOptions.Password) = 0) then
  exit;

     DataOnServer := TIdMultiPartFormDataStream.Create;
     DataOnServer.AddFormField ('user'     , TmpOptions.pOptions.Login);
     DataOnServer.AddFormField ('password' , TmpOptions.pOptions.Password);
     DataOnServer.AddFormField ('version'  , TmpOptions.webOptions.LVER);
  ...
  try
    ...
    HTTPmech.Post(TmpOptions.webOptions.Login,DataOnServer,Response);

        if copy(Response.DataString,1, 11) = 'Old version' then begin
              ExecEventOldVer;
              result:= false; exit
        end;

	if copy(Response.DataString,1, 9) = 'Bad login' then
        begin result:= false; ExecEventBadLogin(); exit end;
	...
	counter := 1; lines_found := 0; cur_str := '';
     ...
     while counter <> len do
        if ResultString[counter] = ':' then begin
			inc(lines_found);
		
			if lines_found = 1 then AuthInfo.GameBuild := StrToInt(cur_str)
			else if lines_found = 2 then AuthInfo.Download := cur_str
			else if lines_found = 3 then AuthInfo.RealLogin := cur_str
			else if lines_found = 4 then AuthInfo.SessionId := cur_str;

			cur_str := ''; inc(counter)
        end
        else begin cur_str := cur_str+ResultString[counter]; inc(counter) end;

	if (AuthInfo.RealLogin <> '') and (AuthInfo.SessionId <> '') then result := true;
...
if not result then
        if Length(ResultString) = 0 then ExecEventUnknown('ПУСТАЯ СТРОКА')
        else                             ExecEventUnknown(ResultString);
...
begin
  ...
  try
   ...
   HTTPmech.Get(PrivateGetOptions.webOptions.Distr+fname, Strm);

   SetString(result,PChar(Strm.memory),Strm.Size);

  except on exception do  result := 'none';
  end;
...
begin
 ...
 if Length(depass) > 0 then begin

    if not TryStrToInt(DeCryptMe(depass,IntToStr(PassBase)), TmpPass) then exit;

    if not TryStrToInt(Decrypt(version,TmpPass), TmpPass) then exit;

 end
 else begin

    if not TryStrToInt(version, TmpPass) then exit;
...
begin
  ...
  if Length(AutoConfigLink) = 0 then exit;
  ...
  try
   ...
   if not ValidConfig(DownloadOptions.sysOptions.Depass, DownloadOptions.webOptions.LVER) then begin
    OldLauncherTicket;
    result := false
   end
   else begin
  ...
  if not result then ExecEventConnectErr;
...
begin
 ...
 if (not FileExists(dirNative + 'jinput-dx8.dll')) or
    ...
    (not FileExists(dirNative + 'OpenAL64.dll')) then result := false
 else result := true;
...
begin
 ...
 if (not FileExists(dirBin + 'jinput.jar')) or
    ...
    (not FileExists(dirBin + 'minecraft.jar')) or
    (not IsNativesWinInstalled) then result := false
 else result := true;
...
begin
 ...
 // result := false;
  ...
  if hash = 'none' then exit;

  //messageBox(0,pChar(GetMD5Hash(fname) + '| ' + hash + ' | '+ fname),'Ошибка', mb_OK);

  if CompareText(trim(GetMD5Hash(fname)), trim(hash)) = 0 then result := false
  else result := true;
...
begin
  ...
  if Length(tmpOptions.webOptions.Distr_list) = 0 then begin
      result := false;
      exit;
  end;
  ...
  for i:=0 to High(FileList) do
       if not DownloadFile(FileList[i]) then begin
...
begin
  ...
  if Length(TmpOptions.gOptions.Game) = 0 then begin
     result := false;
     exit;
  end;
   
  if ExtractFileExt(fname) = '.zip' then zip := true
  else zip := false;

{
 ...
 Пропуск на скачивание как в оригинальном лаунчере, эти данные могут появиться только после авторизации
  ...
  if (Length(AuthInfo.Download) > 0) and (Length(TmpOptions.pOptions.Login) > 0) then
  HTTPParams := '?user=' + TmpOptions.pOptions.Login + '&ticket=' + AuthInfo.Download;
...

{
 ...
 Библиотеки для разных операционных систем. В данной версии только для windows обработка
  ...
  if (fname = 'windows_natives.jar') and (IsNativesWinInstalled) then begin
  ...
  
{
 ...
 Исключение zip, если отсутствует проверка хэша zipа , проверка проходит по запросу хэша minecraft.jar'a
  ...
  if (not zip) and (fname <> 'windows_natives.jar') and
     (not NeedToFileUpdate(TmpOptions.gOptions.Game + 'bin\' + fname, Download.hash)) then exit;

  if (Download.hash <> 'none') and (zip) and (not NeedToFileUpdate(RootDir + 'TEMP\' + fname, Download.hash)) then begin

     if not ZipUnzip(RootDir,fname,false) then result := false
     else begin

         Options.gOptions.MineHash := CryptMe(GetMD5Hash(TmpOptions.gOptions.Game + 'bin\minecraft.jar'),TmpOptions.sysOptions.Depass);
         SaveOptions;
...

{
  ...
  if (Download.hash = 'none') and (zip) then begin

        cur_mineHash := GetMD5Hash(TmpOptions.gOptions.Game + 'bin\minecraft.jar'); { хэш текущего исполняемого файла }
        web_mineHash := DownloadMD5(fname + '.md5');

        Options.gOptions.MineHash := CryptMe(web_mineHash,TmpOptions.sysOptions.Depass); { хэш с сайта }
        SaveOptions;

        if CompareText(trim(web_mineHash), trim(cur_mineHash)) = 0 then exit;
 ...
 // ForceDirectories
  if not DirectoryExists(tmp + 'TEMP\') then CreateDirectory(PAnsiChar(tmp + 'TEMP\'), nil);
  if not DirectoryExists(TmpOptions.gOptions.Game) then ForceDirectories(TmpOptions.gOptions.Game);
...

{
 ...
 Обработка скачанного файла
  ...
  if result = true then begin
       ...
       if fname = 'windows_natives.jar' then ZipUnzip(tmp,fname,true)
       else if zip then begin

               if not ZipUnzip(tmp,fname,false) then result := false
               else begin
          ...
          if not DirectoryExists(TmpOptions.gOptions.Game + 'bin\') then ForceDirectories(TmpOptions.gOptions.Game + 'bin\');

          if fname = 'minecraft.jar' then begin

             Options.gOptions.MineHash := CryptMe(Download.hash,TmpOptions.sysOptions.Depass);
             SaveOptions;

           end;

          if not CopyFile(PChar(tmp + 'TEMP\' + fname),
		          PChar(TmpOptions.gOptions.Game + 'bin\' + fname),false) then result := false;
...
begin
  ...
  with ZipModule do begin

       BaseDirectory := GameDir;

       if fname = 'windows_natives.jar' then BaseDirectory := GameDir + 'bin\natives\';

       if not DirectoryExists(BaseDirectory) then ForceDirectories(BaseDirectory);
  ...
  if UnzipFailTrigger then result := false;

  if (result) and (fname = 'client.zip') and (not FileExists(GameDir + 'bin\minecraft.jar')) then result := false;

  FreeAndNil(ZipModule);

  if delete then  DeleteFile(rootdir + 'TEMP\' + fname);

end;

procedure TCore.UnzipItemWork(Sender: TObject; Item : TAbArchiveItem; Progress : Byte; var Abort : Boolean);
begin
        ExecEventUnarchItem(Item, Progress, Abort);
end;


procedure TCore.UnzipItemFail(Sender: TObject; Item : TAbArchiveItem; ProcessType : TAbProcessType; ErrorClass : TAbErrorClass; ErrorCode : Integer);
begin
        UnzipFailTrigger := false;
        ExecEventFailItem(Item, ProcessType, ErrorClass, ErrorCode);
end;

procedure TCore.HTTPmechWork(Sender: TObject; AWorkMode: TWorkMode; const AWorkCount: Integer);
begin
	Download.currentsize := AWorkCount;
        ExecEventDwnProcess(Download);
end;

procedure TCore.HTTPmechWorkBegin(Sender: TObject; AWorkMode: TWorkMode; const AWorkCountMax: Integer);
begin
...
begin
  ...
  if FileExists(OptionsFile) = false then begin
    result := false;
    exit
  end;
  ...
  if not ValidConfig(TmpOptions.sysOptions.Depass, TmpOptions.webOptions.LVER) then result := false
  else begin
             result := true;
             Options := TmpOptions;
  end;    
...
begin
  ...
  if not ValidConfig(PrefabSet.sysOptions.Depass ,PrefabSet.webOptions.LVER) then begin
    OldLauncherTicket;
    result := false;
    CloseFile(FOptions);
    exit;
...
begin
 ...
 if not DirectoryExists(OptionsDir) then CreateDir(OptionsDir);
...
begin
 ...
 if Length(Options.sysOptions.Depass) > 0 then begin
        ...
        decOptions.pOptions.Password     := Decrypt(Options.pOptions.Password     , tempPass);
        ...
        decOptions.webOptions.Distr_list := Decrypt(Options.webOptions.Distr_list , tempPass);
        decOptions.webOptions.LVER       := Decrypt(Options.webOptions.LVER       , tempPass);
        decOptions.gOptions.MineHash     := Decrypt(Options.gOptions.MineHash     , tempPass);
...
var decOptions: globalOptions;
begin

 decOptions := PrivateGetOptions;

 if decOptions.sysOptions.Forbit then begin
...

{
 ...
 Если системные поля менять запрещено, то перезаписываем текущими
    ...
    if Options.sysOptions.Forbit then begin
...
Если добавился ключ шифрования, то шифруем важные данные
    ...
    if Length(newOptions.sysOptions.Depass) > 0 then begin
       ...
       if Options.sysOptions.Forbit then
         ...
         newOptions.sysOptions.Depass := Encrypt(newOptions.sysOptions.Depass , PassBase);

       end;

        newOptions.pOptions.Login    := Encrypt(newOptions.pOptions.Login, tempPass);
        newOptions.pOptions.Password := Encrypt(newOptions.pOptions.Password, tempPass);

       if not Options.sysOptions.Forbit then begin

        newOptions.webOptions.Login      :=  Encrypt(newOptions.webOptions.Login      , tempPass);
        newOptions.webOptions.Distr      :=  Encrypt(newOptions.webOptions.Distr      , tempPass);
        newOptions.webOptions.Distr_list :=  Encrypt(newOptions.webOptions.Distr_list , tempPass);
        newOptions.webOptions.LVER       :=  Encrypt(newOptions.webOptions.LVER       , tempPass);
        newOptions.gOptions.MineHash     :=  Encrypt(newOptions.gOptions.MineHash     , tempPass);
